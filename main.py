            ...
            for bookie in m['bookmakers']:
                ...
                if h2h:
                    for out in h2h['outcomes']:
                        ...
                        edge = (p * out['price']) - 1
                        if edge > 0:
                            all_potential_bets.append({'Čas': start_time, 'Liga': liga, 'Zápas': f"{api_h} vs {api_a}", 'Typ': '⚖️ 1X2', 'Tip': out['name'], 'Kurz': out['price'], 'Edge': edge, 'Pravd': p})
                ...
                if totals:
                    for out in totals['outcomes']:
                        if out.get('point') == goly_limit:
                            ...
                            edge = (p * out['price']) - 1
                            if edge > 0:
                                all_potential_bets.append({'Čas': start_time, 'Liga': liga, 'Zápas': f"{api_h} vs {api_a}", 'Tip': f"{out['name']} {goly_limit}", 'Typ': '⚽ Góly', 'Kurz': out['price'], 'Edge': edge, 'Pravd': p})
