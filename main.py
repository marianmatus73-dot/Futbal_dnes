...
# KOMPLETNÝ ZOZNAM LÍG (Futbal, Hokej, Tenis)
LIGY_CONFIG = {
...
async def vyhodnot_vysledky(session):
    ...
    try:
        ...
        if df.empty: return ""
        updates = 0
        # Vyhodnocujeme len futbal z football-data.co.uk (E0 = Premier League ako príklad)
...
async def analyzuj():
    async with aiohttp.ClientSession() as session:
        log_vysledkov = await vyhodnot_vysledky(session)
        ...
        for liga, content in csv_results:
            ...
            limit_h = 36 if cfg['sport'] == 'hokej' else 24
            async with session.get(f'https://api.the-odds-api.com/v4/sports/{cfg["api"]}/odds/', 
                ...
                for m in await r.json():
                    try:
                        ...
                        if not (now_utc <= m_t <= now_utc + timedelta(hours=limit_h)): continue
                                ...
                                for out in mk['outcomes']:
                                    ...
                                    if lbl in probs:
                                        ...
                                        if min_e <= edge <= 0.45:
                                            ...
                                            all_bets.append({'Zápas':f"{c1} vs {c2}", 'Tip':lbl, 'Kurz':out['price'], 'Edge':f"{round(edge*100,1)}%", 'Vklad':f"{vklad}€", 'Sport':cfg['sport']})
                    except: continue

        if all_bets:
            final = pd.DataFrame(all_bets).sort_values('Edge', ascending=False).drop_duplicates(subset=['Zápas', 'Tip']).to_dict('records')
            uloz_a_posli(final, log_vysledkov)
...
def uloz_a_posli(bets, log_v):
    ...
    html = f"{log_v}<h3>🎯 Nové Value Bets</h3><table border='1' style='border-collapse:collapse; width:100%;'><tr><th>Šport</th><th>Zápas</th><th>Tip</th><th>Kurz</th><th>Edge</th><th>Vklad</th></tr>"
    for b in bets:
