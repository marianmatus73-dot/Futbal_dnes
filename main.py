...
try:
    ...
    MultisportLearningManager = None
    _multisport_learning_import_error = exc
else:
    _multisport_learning_import_error = None
...
def run_complete_multisport_learning_bundle(
    settings: Settings,
    selected_sports: list[str],
) -> dict | None:
    """Run the shared complete learning package for non-football sports.

    The package uses the same SQLite database as the main application. It is
    optional and fails safely when the bundle has not yet been copied into
    ``core/multisport_learning``.
    """
    if os.getenv("MULTISPORT_LEARNING_BUNDLE_ENABLED", "1") != "1":
        log.info("Complete multisport learning bundle disabled.")
        return None

    requested = [
        sport
        for sport in selected_sports
        if sport in COMPLETE_LEARNING_SPORTS
    ]

    if not requested:
        log.info("No supported sports selected for complete learning bundle.")
        return None

    if MultisportLearningManager is None:
        log.warning(
            "Complete multisport learning bundle unavailable: %s",
            _multisport_learning_import_error,
        )
        return {
            "sports": {},
            "sports_completed": 0,
            "ready": 0,
            "status": "UNAVAILABLE",
            "error": str(_multisport_learning_import_error),
        }

    manager = MultisportLearningManager(str(db_path(settings)))
    result = manager.run_all(requested)

    export_dir = Path(os.getenv("EXPORT_DIR", "exports"))
    export_dir.mkdir(parents=True, exist_ok=True)
    report_path = export_dir / "multisport_learning_report.json"
    ...
    log.info(
        "Complete multisport learning bundle finished: "
        "sports=%s ready=%s status=%s report=%s",
...
async def run() -> None:
    ...
    # COMPLETE MULTISPORT LEARNING BUNDLE
    multisport_learning_result = None
    try:
        selected_sport_names = [sport.name for sport in selected]
        multisport_learning_result = run_complete_multisport_learning_bundle(
            settings,
            selected_sport_names,
        )
    except Exception:
        log.exception("Complete multisport learning bundle failed")
    ...
    if multisport_learning_result is not None:
        report_text += "\n\n=== COMPLETE MULTISPORT LEARNING BUNDLE ===\n"
        report_text += (
            f"Sports completed: "
            f"{multisport_learning_result.get('sports_completed', 0)}\n"
            f"Sports ready: {multisport_learning_result.get('ready', 0)}\n"
            f"Status: {multisport_learning_result.get('status', 'UNKNOWN')}\n"
        )

        for sport_name, sport_result in (
            multisport_learning_result.get("sports", {}).items()
        ):
            health = sport_result.get("stages", {}).get("ai_health", {})
            evaluation = sport_result.get("stages", {}).get("evaluation", {})
            report_text += (
                f"- {sport_name}: {sport_result.get('status', 'UNKNOWN')} | "
                f"samples={health.get('training_samples', 0)} | "
                f"maturity={health.get('maturity', 'UNKNOWN')} | "
                f"brier={evaluation.get('brier_score', 'n/a')}\n"
            )
