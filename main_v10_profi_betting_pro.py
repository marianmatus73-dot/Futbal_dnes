...
APP_VERSION = "10.1-profi-enhanced"
DB_SCHEMA_VERSION = 3
...
MARKET_BLEND_WEIGHT = env_float("MARKET_BLEND_WEIGHT", 0.15)
MIN_MARKET_EDGE = env_float("MIN_MARKET_EDGE", 0.015)  # edge vs no-vig consensus, model sanity guard
...
def update_clv_metrics() -> int:
    ...
    with db_connect() as conn:
        ...
        for bet_id, datum_iso, league, home, away, tip, market, taken, bookmaker in rows:
            ...
            if taken <= 1.01:
                continue
            kickoff = pd.to_datetime(datum_iso, errors="coerce", utc=True)
            if pd.isna(kickoff):
                continue
...
async def settle_results(session: aiohttp.ClientSession) -> int:
    ...
    for league_name, cfg in LIGY.items():
        ...
        for row_id, datum_iso, zapas, home, away, tip, _league in candidate_rows:
            ...
            if datum_iso:
                parsed_dt = pd.to_datetime(datum_iso, errors="coerce", utc=True)
...
def blended_probability(model_prob: float, market_prob: Optional[float]) -> float:
    w = min(max(MARKET_BLEND_WEIGHT, 0.0), 0.60)
    if market_prob is None or market_prob <= 0:
        return model_prob
    return float((1.0 - w) * model_prob + w * market_prob)
            ...
            for bookmaker in match.get("bookmakers", []):
                ...
                for market in bookmaker.get("markets", []):
                    ...
                    for outcome in market.get("outcomes", []):
                        ...
                        # Sanity guard: reject cases where our model barely disagrees
                        # with no-vig consensus. This reduces random/stale quotes.
