...
class NFLModule(SportModule):
    ...
    def _extra_adjustment(self, home: str, away: str, selection: str, league: str) -> float:
        if os.getenv("NFL_HOME_ADV_ENABLED", "1") != "1":
            return 0.0

        home_adv = float(os.getenv("NFL_HOME_PROB_ADV", "0.015"))
    ...
    async def scan(self, settings: Settings) -> SportResult:
        ...
        min_books = int(os.getenv("MIN_NFL_BOOKMAKERS", "3"))
        top_n = int(os.getenv("TOP_N_REPORT", "8"))
        grade_min_samples = int(os.getenv("NFL_BOOKMAKER_GRADE_MIN_SAMPLES", "20"))
        ...
        for sport_key in clean_sport_keys:
            ...
            for event in data:
                ...
                for bookmaker, selection, odds in best_outlier_prices(bookmakers):
                    ...
                    probability_reason = "original nfl probability"
        ...
        return SportResult(
            ...
            bets=bets[:top_n],
            message=(
                "NFL v12: meta-model/fallback/CLV/ELO market model. "
                f"Settled: {settled}. "
