...
class EsportsModule(SportModule):
    ...
    async def scan(self, settings: Settings) -> SportResult:
        ...
        configured_keys = [
            item.strip()
            for item in os.getenv("ESPORTS_SPORT_KEYS", "").split(",")
            if item.strip()
        ]

        clean_sport_keys = list(configured_keys)

        if os.getenv("SPORT_KEY_AUTO_DISCOVERY", "1") == "1":
            discovered_keys = await discover_active_sport_keys(
            ...
            if discovered_keys:
                clean_sport_keys = discovered_keys
        ...
        if not clean_sport_keys:
            return SportResult(
                ...
                bets=[],
                message=(
                    "Esports v12: Odds API nevrátil žiadne aktívne "
                    "esports turnaje. Modul bol bezpečne preskočený."
                ),
            )
        ...
        for sport_key in clean_sport_keys:
            ...
            for event in data:
                ...
                for bookmaker, selection, odds in best_outlier_prices(bookmakers):
                    ...
                    fallback_probability = max(0.01, min(0.99, prob_market + elo_adj + extra_adj))
                    ...
                    mc_preview = simulate_single_bet(probability=fallback_probability, odds=odds)
                    probability_source = "FALLBACK"
                    probability_reason = "original esports probability"
                    try:
                    ...
                    adjusted_edge = (
                        edge * grade * current_sport_weight
                        * current_bookmaker_weight * current_league_weight
                    )
                    ...
                    if edge < settings.min_edge:
                        ...
                        self._audit(
                            settings, sport_key, event_name, selection, bookmaker, odds,
                            prob_market, edge, "BLOCK", f"edge below minimum; probability_source={probability_source}; probability_reason={probability_reason}"
                        )
                        continue
                    ...
                    if edge > settings.max_edge:
                        ...
                        self._audit(
                            settings, sport_key, event_name, selection, bookmaker, odds,
                            prob_market, edge, "BLOCK", f"edge above max guard; probability_source={probability_source}; probability_reason={probability_reason}"
                        )
                        continue
                    ...
                    if odds > settings.max_odds:
                        ...
                        self._audit(
                            settings, sport_key, event_name, selection, bookmaker, odds,
                            prob_market, edge, "BLOCK", f"odds above max odds; probability_source={probability_source}; probability_reason={probability_reason}"
                        )
                        continue
                    ...
                    if stake <= 0:
                        ...
                        self._audit(
                            settings, sport_key, event_name, selection, bookmaker, odds,
                            prob_market, edge, "BLOCK", f"stake <= 0; probability_source={probability_source}; probability_reason={probability_reason}"
                        )
                        continue
        ...
        return SportResult(
            ...
            bets=bets[:top_n],
            message=(
                "Esports v12: meta-model/fallback/CLV/ELO market model. "
                f"Settled: {settled}. "
