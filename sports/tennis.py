...
class TennisModule(SportModule):
    ...
    # Doplňujúca metóda pre hashovanie zdrojov
    ...
    def _save_bet(self, settings: Settings, bet: Bet) -> None:
        # Pôvodná logika ukladania stávok
        with self._connect(settings) as conn:
            conn.execute("INSERT OR IGNORE INTO sport_bets (sport, league, event, selection, odds, stake, bookmaker, result) VALUES (?, ?, ?, ?, ?, ?, ?, 'OPEN')",
    ...
    async def scan(self, settings: Settings) -> SportResult:
        ...
        if os.getenv("SPORT_KEY_AUTO_DISCOVERY", "1") == "1":
            ...
            # --- APIDEBUG LOGIKA ---
            log.info("APIDEBUG: Všetky dostupné tenisové kľúče z API: %s", active_keys)
            
            clean_sport_keys = filter_active_keys(clean_sport_keys, active_keys)
            log.info("APIDEBUG: Finálny zoznam na skenovanie: %s", clean_sport_keys)
            # -----------------------
