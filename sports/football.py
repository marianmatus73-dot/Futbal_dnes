...
def league_v13_parameters(
    ...
    """
    Convert registry home-advantage intensity into:
    - xG multiplier,
    - ELO home advantage,
    - league average xG.

    These are conservative defaults until league-specific history is built.
    """
    ...
    home_advantage_xg = max(
        1.00,
        min(1.15, 1.00 + ha * 0.20),
    )
    home_advantage_elo = max(
        25.0,
        min(100.0, 30.0 + ha * 100.0),
    )

    league_average_xg = 1.40 if tier == "A" else 1.35
        ...
        for sport_key in clean_sport_keys:
            ...
            for event in data:
                ...
                (
                    ...
                    home_advantage_xg,
                    home_advantage_elo,
                ) = league_v13_parameters(sport_key)
