...
from core.football_scan_optimizer_v14 import (
...
class FootballModule(SportModule):
    ...
    def _save_snapshot(
        ...
        optimized_rows, optimizer_stats = (
            optimize_h2h_snapshot_rows(
                bookmakers=bookmakers,
                commence_time=commence_time,
                near_window_hours=float(
                    os.getenv(
                        "FOOTBALL_SNAPSHOT_NEAR_HOURS",
                        "72",
                    )
                ),
                near_books_per_selection=int(
                    os.getenv(
                        "FOOTBALL_SNAPSHOT_NEAR_BOOKS",
                        "3",
                    )
                ),
                far_books_per_selection=int(
                    os.getenv(
                        "FOOTBALL_SNAPSHOT_FAR_BOOKS",
                        "1",
                    )
                ),
            )
        ...
        # Hourly bucket prevents identical reruns from creating a full new
        # copy of distant-event snapshots.
        captured_bucket = (
    ...
    async def scan(self, settings: Settings) -> SportResult:
        ...
        for sport_key in clean_sport_keys:
            ...
            for event in data:
                ...
                optimized_candidates, candidate_stats = (
                    optimize_candidate_prices(
                        best_outlier_prices(bookmakers)
                    )
                )

                candidate_rows_raw += candidate_stats.valid_rows
                candidate_rows_optimized += (
                    candidate_stats.unique_selections
                )

                for candidate_price in optimized_candidates:
                    bookmaker = candidate_price.bookmaker
                    selection = candidate_price.selection
                    odds = candidate_price.odds
                    ...
                    if edge < settings.min_edge:
                        ...
                        explain_and_save_football_decision_v15(
                            settings,
                            source_hash=rejection_source_hash,
                            features=features,
                            meta_prediction=meta_prediction,
                            decision="BLOCK",
                            adjusted_edge=adjusted_edge,
                            confidence=features.confidence_input,
                            risk="high",
                            rejection_reason="edge below minimum",
                        )
                        self._audit(
                            settings,
                            sport_key,
                    ...
                    if edge > settings.max_edge:
                        ...
                        explain_and_save_football_decision_v15(
                            ...
                            confidence=features.confidence_input,
                            risk="high",
                            rejection_reason="edge above max guard",
                        )
                        self._audit(
                            settings,
                            sport_key,
                    ...
                    if odds > settings.max_odds:
                        ...
                        explain_and_save_football_decision_v15(
                            ...
                            confidence=features.confidence_input,
                            risk="high",
                            rejection_reason="odds above maximum",
                        )
                        self._audit(
                            settings,
                            sport_key,
                    ...
                    if stake <= 0:
                        ...
                        explain_and_save_football_decision_v15(
                            ...
                            confidence=features.confidence_input,
                            risk="high",
                            rejection_reason="stake not positive",
                        )
                        self._audit(
                            settings,
                            sport_key,
                    ...
                    explain_and_save_football_decision_v15(
                        ...
                        risk=(
                            "low"
                            if features.confidence_input >= 0.80
                            else "medium"
                            if features.confidence_input >= 0.65
                            else "high"
                        ),
                    )
