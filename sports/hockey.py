...
class HockeyModule(SportModule):
    name = "hockey"
    ...
    def _save_snapshot(self, settings: Settings, sport_key: str, event_name: str, home: str, away: str, bookmakers: list[dict]) -> int:
        ...
        for bookmaker in bookmakers:
            ...
            for market in bookmaker.get("markets", []):
                ...
                for outcome in market.get("outcomes", []):
                    ...
                    if odds <= 1.01:
                        continue
                    source_hash = make_hash(captured_at, self.name, sport_key, event_name, book, "h2h", selection, odds)
                    rows.append((captured_at, self.name, sport_key, event_name, home, away, book, "h2h", selection, odds, source_hash))
        ...
        with self._connect(settings) as conn:
            ...
            conn.executemany("""
                ...
                (captured_at, sport, league, event, home_team, away_team, bookmaker, market, selection, odds, source_hash)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, rows)
    ...
    def _audit(self, settings: Settings, sport_key: str, event_name: str, selection: str, bookmaker: str, odds: float,
        ...
        with self._connect(settings) as conn:
            conn.execute("""
                ...
                (sport, league, event, selection, bookmaker, odds, prob_market, edge, decision, reason)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (self.name, sport_key, event_name, selection, bookmaker, odds, prob_market, edge, decision, reason))
    ...
    async def scan(self, settings: Settings) -> SportResult:
        ...
        configured_keys = os.getenv(
            "HOCKEY_SPORT_KEYS",
            "icehockey_nhl,icehockey_ahl,icehockey_sweden_hockey_league,icehockey_sweden_allsvenskan,icehockey_liiga,icehockey_mestis",
        ).split(",")

        clean_sport_keys = [s.strip() for s in configured_keys if s.strip()]

        if os.getenv("SPORT_KEY_AUTO_DISCOVERY", "1") == "1":
            active_keys = await discover_active_sport_keys(settings.odds_api_key, ["Ice Hockey"])
        ...
        grade_min_samples = int(os.getenv("HOCKEY_BOOKMAKER_GRADE_MIN_SAMPLES", "20"))
        ...
        for sport_key in clean_sport_keys:
            ...
            for event in data:
                ...
                for bookmaker, selection, odds in best_outlier_prices(bookmakers):
                    ...
                    hockey_adj = poisson_hockey_adjustment(settings, home, away, selection)

                    fallback_probability = max(
                        0.01,
                        min(0.99, prob_market + elo_adj + hockey_adj),
                    )
                    ...
                    probability_reason = "original hockey probability"
                    try:
                    ...
                    if edge < settings.min_edge:
                        blocked += 1
                        self._audit(settings, sport_key, event_name, selection, bookmaker, odds, prob_market, edge, "BLOCK", f"edge below minimum; probability_source={probability_source}; probability_reason={probability_reason}")
                        continue
                    if edge > settings.max_edge:
                        blocked += 1
                        self._audit(settings, sport_key, event_name, selection, bookmaker, odds, prob_market, edge, "BLOCK", f"edge above max guard; probability_source={probability_source}; probability_reason={probability_reason}")
                        continue
                    if odds > settings.max_odds:
                        blocked += 1
                        self._audit(settings, sport_key, event_name, selection, bookmaker, odds, prob_market, edge, "BLOCK", f"odds above max odds; probability_source={probability_source}; probability_reason={probability_reason}")
                        continue

                    stake = round(kelly_stake(prob_final, odds, settings) * grade, 2)
                    if stake <= 0:
                        blocked += 1
                        self._audit(settings, sport_key, event_name, selection, bookmaker, odds, prob_market, edge, "BLOCK", f"stake <= 0; probability_source={probability_source}; probability_reason={probability_reason}")
                        continue
        ...
        return SportResult(
            ...
            bets=bets[:top_n],
            message=(
                "Hockey v12: meta-model/fallback/CLV/ELO/Poisson model. "
                f"Settled: {settled}. "
