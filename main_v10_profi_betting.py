...
APP_VERSION = "10.6-profi-quant-meta-elo-clv"
DB_SCHEMA_VERSION = 4
...
AI_FILTER_ENABLED = env_int("AI_FILTER_ENABLED", 1)
...
LONGSHOT_MAX_ODDS = env_float("LONGSHOT_MAX_ODDS", 5.50)
LONGSHOT_MIN_EDGE = env_float("LONGSHOT_MIN_EDGE", 0.095)
            ...
            for bookmaker in match.get("bookmakers", []):
                ...
                for market in bookmaker.get("markets", []):
                    ...
                    for outcome in market.get("outcomes", []):
                        ...
                        stake = max(0.0, kelly_stake(prob, price) * max(0.35, 1.0 - penalty / 10.0) * meta_mult)
                        stake = round(min(stake, BANK * MAX_STAKE_PCT), 2)
                        if stake < MIN_STAKE:
                            continue
                        base_score = value_score(prob, price, edge, lh, la) - penalty + ((meta_conf - 0.60) * 4.0)
                        ai_prob = None
                        score = base_score
...
def export_csv(bets: Sequence[BetCandidate]) -> Optional[Path]:
    ...
    fields = [
        ...
        "lh", "la", "vklad", "bookmaker", "ai_prob", "elo_diff", "line_move_pct", "meta_confidence", "score", "explanation"
    ]
