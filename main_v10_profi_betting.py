...
KELLY_FRAC = env_float("KELLY_FRAC", 0.08)
MAX_STAKE_PCT = env_float("MAX_STAKE_PCT", 0.03)
MIN_STAKE = env_float("MIN_STAKE", 1.0)
MIN_EDGE = env_float("MIN_EDGE", 0.04)
MAX_EDGE = env_float("MAX_EDGE", 0.25)
MIN_AI_PROB = env_float("MIN_AI_PROB", 0.52)
...
TOP_N_REPORT = env_int("TOP_N_REPORT", 25)
MAX_BETS_PER_DAY = env_int("MAX_BETS_PER_DAY", 12)
MAX_DAILY_EXPOSURE_PCT = env_float("MAX_DAILY_EXPOSURE_PCT", 0.10)
MAX_MATCH_EXPOSURE_PCT = env_float("MAX_MATCH_EXPOSURE_PCT", 0.035)
MAX_LEAGUE_EXPOSURE_PCT = env_float("MAX_LEAGUE_EXPOSURE_PCT", 0.06)
...
class BetCandidate:
    ...
    def fair_odds(self) -> float:
        return round(1.0 / self.prob_model, 3) if self.prob_model > 0 else 0.0
...
def value_score(prob: float, odds: float, edge: float, lh: float, la: float) -> float:
    ...
    """
    odds_penalty = max(0.0, odds - 3.5) * 1.6
    edge_penalty = max(0.0, edge - 0.18) * 35
    goal_uncertainty = abs(lh - la) * 0.25
    return round(edge * 100 + prob * 12 - odds_penalty - edge_penalty + goal_uncertainty, 4)
...
async def settle_results(session: aiohttp.ClientSession) -> int:
    with db_connect() as conn:
        rows = conn.execute("""
            SELECT id, zapas, home_team, away_team, tip
            FROM bets
    ...
    for league_name, cfg in LIGY.items():
        ...
        for row_id, zapas, home, away, tip in rows:
            ...
            if not csv_h or not csv_a:
                continue
            m = df[(df["HomeTeam"] == csv_h) & (df["AwayTeam"] == csv_a)]
            if m.empty:
                continue
            r = m.iloc[0]
...
def train_ai_model() -> Tuple[Optional[Any], List[str]]:
    features = ["edge", "kurz", "lh", "la", "prob_model", "score"]
                ...
                for market in bookmaker.get("markets", []):
                    ...
                    for outcome in market.get("outcomes", []):
                        ...
                        if not is_reasonable_candidate(prob, price, edge):
                            continue
                        stake = kelly_stake(prob, price)
                        if stake < MIN_STAKE:
                            continue
                        base_score = value_score(prob, price, edge, lh, la)
                        ai_prob = None
                        score = base_score
...
def export_csv(bets: Sequence[BetCandidate]) -> Optional[Path]:
    ...
    fields = [
        ...
        "edge", "implied_prob", "fair_odds", "ev_eur", "risk_level",
...
def export_json(bets: Sequence[BetCandidate]) -> Optional[Path]:
    ...
    for b in bets:
        ...
        d.update({"implied_prob": b.implied_prob, "fair_odds": b.fair_odds, "ev_eur": b.ev_eur, "risk_level": b.risk_level})
        rows.append(d)
    path.write_text(pd.Series(rows).to_json(orient="values", force_ascii=False, indent=2), encoding="utf-8")
    return path
...
def format_report(bets: Sequence[BetCandidate], settled: int, inserted: int) -> str:
    lines = [
        ...
        f"Bank: {BANK:.2f} EUR | Kelly fraction: {KELLY_FRAC} | Max stake: {MAX_STAKE_PCT*100:.1f}%",
    ...
    for i, b in enumerate(bets, 1):
        ...
        lines.append(
            ...
            f"| Fair {b.fair_odds} | P {b.prob_model:.1%} | Edge {b.edge*100:.1f}% "
            f"| EV {b.ev_eur:.2f} EUR | Stake {b.vklad:.2f} EUR | Risk {b.risk_level} "
            f"| {b.bookmaker}{ai}"
        )
    return "\n".join(lines)
...
def apply_args(args: argparse.Namespace) -> None:
    global ACTIVE_LEAGUES, DRY_RUN, SETTLE_ONLY, NO_EMAIL, BACKTEST_ONLY, BACKTEST_DAYS, MIN_EDGE, TOP_N_REPORT, BANK
