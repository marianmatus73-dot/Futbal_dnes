...
def load_rows(
    ...
    selected = [
        ...
        expression(schema.model_probability, "model_probability"),
        expression(schema.market_probability, "market_probability"),
...
def calculate(rows) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        elif normalized == "WIN" and stake is not None and odds is not None:
            profit += stake * max(odds - 1.0, 0.0)
        elif normalized == "LOSS" and stake is not None:
            profit -= stake

        probability = as_float(row["model_probability"])
        if probability is not None and normalized in {"WIN", "LOSS"}:
