...
def build_football_features(
    ...
    *,
    ...
    # Conservative consensus: Dixon-Coles leads, ELO and form support it.
    dixon_weight = 0.50
    elo_weight = 0.30
    form_weight = 0.20

    model_consensus_probability = (
        dixon_selection_probability * dixon_weight
        + elo_selection_probability * elo_weight
        + form_selection_probability * form_weight
    )

    # Shrink towards the market when internal data reliability is weak.
    market_shrink = 0.20 + (1.0 - reliability_input) * 0.35

    model_consensus_probability = (
        model_consensus_probability * (1.0 - market_shrink)
        + market_selection_probability * market_shrink
    )

    model_consensus_probability = clamp(
        model_consensus_probability,
        0.01,
        0.99,
    )

    model_dispersion = _dispersion(model_probabilities)
    market_model_gap = (
        model_consensus_probability
