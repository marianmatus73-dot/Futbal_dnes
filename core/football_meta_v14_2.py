...
_MODEL_CACHE: dict[tuple[str, str], "FootballMetaModel"] = {}
...
class FootballMetaModel:
    ...
    def predict(
        ...
        try:
            ...
            reason = (
                "meta=fallback_prediction_error; "
...
def get_football_meta_model(
    *,
    ...
    """
    Return one cached meta-model instance per process.

    A football scan can evaluate thousands of bookmaker selections. Loading
    metadata and checking the pickle path for every selection is unnecessary
    and produces repetitive fallback diagnostics.
    """
    key = (str(model_path), str(metadata_path))

    if force_reload or key not in _MODEL_CACHE:
        _MODEL_CACHE[key] = FootballMetaModel(
...
def clear_football_meta_model_cache() -> None:
    _MODEL_CACHE.clear()
