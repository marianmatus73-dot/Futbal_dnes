...
class FootballSettlementEngine:
    def __init__(
        ...
        *,
        ...
        self.settlement_grace_minutes = max(
            0,
            min(
                720,
                int(
                    os.getenv(
                        "FOOTBALL_SETTLEMENT_GRACE_MINUTES",
                        "120",
                    )
                ),
            ),
    ...
    def _split_eligible_bets(
        self,
        open_bets: list[OpenFootballBet],
    ) -> tuple[list[OpenFootballBet], list[OpenFootballBet]]:
        now = datetime.now(timezone.utc)
        eligible: list[OpenFootballBet] = []
        future: list[OpenFootballBet] = []
        ...
        for bet in open_bets:
            ...
            ready_at = start.timestamp() + (
                self.settlement_grace_minutes * 60
            )

            if now.timestamp() >= ready_at:
                eligible.append(bet)
            else:
                future.append(bet)
    ...
    def _log_diagnostics(
        ...
        if os.getenv("FOOTBALL_SETTLEMENT_CONSOLE_DIAGNOSTICS", "1") != "1":
            return

        score_count = sum(len(games) for games in scores_by_key.values())

        log.info(
            "Football settlement diagnostics: open_bets=%s, score_events=%s",
            len(open_bets),
            score_count,
        )
        ...
        for sport_key, games in sorted(scores_by_key.items()):
            for game in games:
                log.info(
                    "Settlement score event: sport_key=%s | %s | "
                    "%s vs %s | %s-%s",
                    sport_key,
                    game.commence_time,
                    game.home_team,
        ...
        # Log only the best candidate per open bet to avoid excessive output.
        for bet in open_bets:
            games = scores_by_key.get(bet.sport_key, [])

            if not games:
                log.info(
                    "Settlement unmatched: sport_key=%s | %s | "
                    "no completed score events for this sport_key",
                    bet.sport_key,
                    bet.event,
                )
                continue
            ...
            candidates: list[
                tuple[float, float, CompletedFootballGame]
            ] = []
            ...
            log.info(
                "Settlement closest candidate: sport_key=%s | "
                "bet=%s | score=%s vs %s | combined=%.3f | "
                "minimum=%.3f | bet_time=%s | score_time=%s",
                bet.sport_key,
                bet.event,
                game.home_team,
                game.away_team,
    ...
    async def run(self) -> FootballSettlementSummary:
        ...
        eligible_bets, future_bets = self._split_eligible_bets(
            open_bets,
        )

        log.info(
            "Football settlement eligibility: open=%s, eligible=%s, "
            "future_skipped=%s, grace_minutes=%s",
            len(open_bets),
            len(eligible_bets),
            len(future_bets),
            self.settlement_grace_minutes,
        )

        sport_keys = sorted(
            {bet.sport_key for bet in eligible_bets}
        )
        ...
        return FootballSettlementSummary(
            ...
            eligible_bets=len(eligible_bets),
            future_bets_skipped=len(future_bets),
...
if __name__ == "__main__":
    async def _main() -> None:
        ...
        print(
            ...
            f"eligible={summary.eligible_bets}, "
            f"future_skipped={summary.future_bets_skipped}, "
